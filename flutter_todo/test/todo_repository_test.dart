...
void main() {
  ...
  group('TodoRepository CRUD and state modifications', () {
    test(
      'addTodo computes next priority, creates issue on GitHub and updates local list',
      () async {
        final fakeClient = FakeClient((request) {
          if (request.method == 'POST' &&
              request.url.path == '/repos/$testRepo/issues') {
            final bodyMap = jsonDecode((request as http.Request).body);
            expect(bodyMap['title'], 'New Task');
            expect(
              bodyMap['body'],
              contains('<!-- todo-meta: {"priority":0,"deleted":false} -->'),
            );
    ...
    test('updateTodo patches title and body metadata on GitHub', () async {
      ...
      final fakeClient = FakeClient((request) {
        if (request.method == 'PATCH' &&
            request.url.path == '/repos/$testRepo/issues/101') {
    ...
    test(
      'toggleStatus flips active to done and sends closed state to GitHub',
      () async {
        late Map<String, dynamic> capturedPatchBody;
        final fakeClient = FakeClient((request) {
          if (request.method == 'PATCH' &&
    ...
    test(
      'moveUp and moveDown swap priorities and patch both GitHub issues',
      () async {
        final patchedIssues = <String, Map<String, dynamic>>{};
        final fakeClient = FakeClient((request) {
          if (request.method == 'PATCH') {
            final issuePath = request.url.path;
            patchedIssues[issuePath] = jsonDecode(
        ...
        expect(
          patchedIssues['/repos/$testRepo/issues/2']!['body'],
          contains('{"priority":0,"deleted":false}'),
        );
        expect(
          patchedIssues['/repos/$testRepo/issues/1']!['body'],
          contains('{"priority":1,"deleted":false}'),
        );
    ...
    test(
      'deleteTodo closes GitHub issue, sets deleted metadata, and removes from local list',
      () async {
